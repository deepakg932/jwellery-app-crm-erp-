...
const useMakingSubStages = () => {
  ...
  const fetchMakingSubStages = async () => {
    try {
      ...
      toast.error("Failed to load making sub-stages. Please try again.", {
        autoClose: 4000,
      });
    } finally {
      setLoading(false);
  ...
  const addMakingSubStage = async (subStageData) => {
    // Show loading toast
    const toastId = toast.loading("Adding making sub-stage...");
    ...
    const data = {
      ...
      is_active:
        subStageData.is_active !== undefined ? subStageData.is_active : true,
    };
    ...
    try {
      ...
      const response = await axios.post(
        API_ENDPOINTS.createMakingSubStage(),
        data,
      );
      ...
      if (response.data && response.data.success && response.data.data) {
        ...
        const parentStage = makingStages.find(
          (stage) =>
            stage.id === responseData.stage_id ||
            stage._id === responseData.stage_id,
        );
        if (parentStage) {
      ...
      toast.update(toastId, {
        render: "Making sub-stage added successfully!",
        type: "success",
        isLoading: false,
        autoClose: 3000,
      });
      ...
      const errorMsg =
        ...
        err.message ||
        "Failed to add making sub-stage";
      setError(errorMsg);
  ...
  const updateMakingSubStage = async (id, data) => {
    // Show loading toast
    const toastId = toast.loading("Updating making sub-stage...");
    ...
    try {
      ...
      const response = await axios.put(
        API_ENDPOINTS.updateMakingSubStage(id),
        requestData,
      );
      ...
      // Update toast to success
      toast.update(toastId, {
        render: "Making sub-stage updated successfully!",
        type: "success",
        isLoading: false,
        autoClose: 3000,
      });
      ...
      const errorMsg =
        err.response?.data?.message ||
        err.message ||
        "Failed to update making sub-stage";
      setError(errorMsg);
  ...
  const deleteMakingSubStage = async (id) => {
    // Show loading toast
    const toastId = toast.loading("Deleting making sub-stage...");

    try {
      setLoading(true);

      const response = await axios.delete(
        API_ENDPOINTS.deleteMakingSubStage(id),
      );
      console.log("Delete making sub-stage response:", response.data);

      if (response.data && response.data.success) {
        setMakingSubStages((prev) =>
          prev.filter((subStage) => subStage._id !== id),
        );

        // Update toast to success
        toast.update(toastId, {
          render: "Making sub-stage deleted successfully!",
          type: "success",
          isLoading: false,
          autoClose: 3000,
        });
      ...
      const errorMsg =
        err.response?.data?.message ||
        err.message ||
        "Failed to delete making sub-stage";
      setError(errorMsg);
