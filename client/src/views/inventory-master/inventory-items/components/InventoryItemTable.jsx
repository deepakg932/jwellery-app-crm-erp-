...
const InventoryItemTable = () => {
  ...
  const filtered = items.filter(
    (item) =>
      ...
      item.inventory_category_name?.toLowerCase().includes(search.toLowerCase()) ||
...
const formatValues = (item) => {
  if (item.track_by === "weight") {
    return `${item.weight || 0} ${item.unit_name || ""}`;
  } else if (item.track_by === "quantity") {
    return `${item.quantity || 0} ${item.unit_name || ""}`;
  } else if (item.track_by === "both") {
    return `${item.weight || 0} ${item.unit_name || ""} / ${item.quantity || 0} pcs`;
  }
  return "";
};
          ...
          <div className="row align-items-center mb-4">
            <div className="col-md-6">
              ...
              <p className="text-muted mb-0">
                Manage your inventory items in a table layout
              </p>
            </div>
              ...
              {loading && items.length === 0 ? (
                ...
                currentItems.map((item, index) => (
                  <tr key={item._id || index}>
                    ...
                    <td>
                      <span className={`badge fw-semibold ${item.status ? 'bg-success' : 'bg-danger'}`}>
