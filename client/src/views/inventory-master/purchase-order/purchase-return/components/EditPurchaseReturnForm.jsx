...
const EditPurchaseReturnForm = ({ onClose, onSave, returnItem, loading = false }) => {
  ...
  const [selectedPurchaseReceived, setSelectedPurchaseReceived] = useState(null);
  const [purchaseReceivedSearchQuery, setPurchaseReceivedSearchQuery] = useState("");
  const [purchaseReceivedSearchResults, setPurchaseReceivedSearchResults] = useState([]);
  const [showPurchaseReceivedSearchResults, setShowPurchaseReceivedSearchResults] = useState(false);
  ...
  useEffect(() => {
    if (returnItem) {
      const purchaseReceivedId = returnItem.purchase_received_id?._id || returnItem.purchase_received_id || "";
      
      // Find the purchase received from stockIns if available
      const foundPurchaseReceived = stockIns.find(pr => pr._id === purchaseReceivedId);
      
      setFormData({
        purchase_received_id: purchaseReceivedId,
        supplier_id: returnItem.supplier_id?._id || returnItem.supplier_id || "",
        supplier_name: returnItem.supplier_name || returnItem.supplier_id?.name || "",
        branch_id: returnItem.branch_id?._id || returnItem.branch_id || "",
        branch_name: returnItem.branch_name || returnItem.branch_id?.name || "",
        ...
        items: returnItem.items?.map(item => {
          // Calculate available quantities from the original purchase received
          const originalItem = foundPurchaseReceived?.items?.find(prItem => 
            prItem._id === (item.purchase_received_item_id || item.grn_item_id)
          );
          
          const receivedQty = parseFloat(originalItem?.received_quantity) || parseFloat(item.received_quantity) || 0;
          const receivedWt = parseFloat(originalItem?.received_weight) || parseFloat(item.received_weight) || 0;
          const returnedQty = parseFloat(originalItem?.returned_quantity) || parseFloat(item.returned_quantity) || 0;
          const returnedWt = parseFloat(originalItem?.returned_weight) || parseFloat(item.returned_weight) || 0;
          
          const availableQty = Math.max(0, receivedQty - returnedQty);
          const availableWt = Math.max(0, receivedWt - returnedWt);

          return {
            purchase_received_item_id: item.purchase_received_item_id || item.grn_item_id || "",
            inventory_item_id: item.inventory_item_id?._id || item.inventory_item_id || "",
            inventory_item_name: item.inventory_item_id?.name || item.inventory_item_name || "Unknown Item",
            sku_code: item.inventory_item_id?.item_code || item.sku_code || "N/A",
            available_quantity: availableQty,
            available_weight: availableWt,
            received_quantity: receivedQty,
            received_weight: receivedWt,
            unit_id: item.unit_id?._id || item.unit_id || "",
            unit_name: item.unit_name || item.unit_id?.name || "pcs",
        ...
        total_cost: parseFloat(returnItem.total_cost) || 0,
        status: returnItem.status || "pending",
      });

      // Set selected Purchase Received if exists
      if (foundPurchaseReceived) {
        setSelectedPurchaseReceived(foundPurchaseReceived);
      }
    }
  }, [returnItem, stockIns]);
  ...
  const handlePurchaseReceivedSelect = (purchaseReceived) => {
    ...
    const purchaseReceivedItems =
      purchaseReceived.items?.map((item) => {
        ...
        // Check if item already exists in current form
        const existingItem = formData.items.find(
          (existing) => 
            existing.purchase_received_item_id === item._id ||
            existing.inventory_item_id === (inventoryItem._id || item.inventory_item_id)
        );
    ...
    setFormData((prev) => ({
      ...
      total_cost: calculateTotalCost(purchaseReceivedItems),
    }));
  ...
  const handleSubmit = (e) => {
    ...
    const payload = {
      _id: returnItem._id, // Include the ID for update
    ...
    console.log("Updating Purchase Return data:", payload);
    onSave(payload);
  };
      ...
      <div className="modal-dialog modal-dialog-centered modal-xl">
        ...
        >
          <div
            ...
            style={{ zIndex: 1020 }}
          >
            <h5 className="modal-title fw-bold fs-5">Edit Purchase Return</h5>
            <button
              type="button"
              className="btn-close"
              onClick={handleClose}
                  ...
                  <input
                    ...
                    value={
                      selectedPurchaseReceived
                        ...
                        : formData.supplier_name || "Select a Purchase Received to auto-fill supplier"
                    }
                    readOnly
                    disabled={isDisabled}
                  />
                    ...
                    value={
                      selectedPurchaseReceived
                        ? selectedPurchaseReceived.branch_id?.branch_name ||
                          ...
                          "N/A"
                        : formData.branch_name || "Select a Purchase Received to auto-fill branch"
                    }
                    readOnly
                    disabled={isDisabled}
                  />
