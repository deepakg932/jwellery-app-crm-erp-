...
const EditStockINForm = ({ onClose, onSave, grn, loading = false }) => {
  const {
    purchaseOrders,
    suppliers,
    inventoryItems,
  ...
  // Initialize form data when grn prop changes
  useEffect(() => {
    if (grn) {
      const itemsData = grn.items?.map(item => {
        // Extract track_by from item or inventory_item
        let trackBy = item.track_by;
        if (!trackBy && item.inventory_item_id) {
          if (typeof item.inventory_item_id === 'object') {
            trackBy = item.inventory_item_id.track_by || "quantity";
          }
        }
        ...
        return {
          ...
          inventory_item_name: item.inventory_item_id?.item_name || 
                              item.inventory_item_id?.name || 
                              item.inventory_item?.name || 
                              "Unknown Item",
          sku_code: item.inventory_item_id?.sku_code || "N/A",
          track_by: trackBy,
          ordered_quantity: item.ordered_quantity || 0,
          ordered_weight: item.ordered_weight || 0,
          unit_id: item.unit_id || null,
          unit_name: item.unit || item.unit_name || "pcs",
          rate: item.cost || 0,
          ...
          metal_purity: item.metal_purity || item.metal_purities?.[0] || null,
          stone_purity: item.stone_purity || item.stone_purities?.[0] || null,
          
          // Receiving fields
          quantity: trackBy === "quantity" ? (item.received_qty || item.quantity || "") : null,
          weight: trackBy === "weight" ? (item.received_weight || item.weight || "") : null,
      ...
      // Fetch PO details if PO exists
      if (grn.po_id) {
        const po = purchaseOrders.find(p => p._id === (grn.po_id._id || grn.po_id));
        setSelectedPO(po);
      }
    }
  }, [grn, purchaseOrders]);
  ...
  const handleSubmit = (e) => {
    ...
    console.log("Updating GRN data:", payload);
    onSave(payload);
  };
...
export default EditStockINForm;