  ...
  const generatePDF = () => {
    try {
      ...
      import('jspdf').then(({ default: jsPDF }) => {
        ...
        doc.setFontSize(11);
        doc.setTextColor(40, 40, 40);
        doc.text("SERVICE DETAILS", 14, 112);
        doc.setFontSize(10);
        doc.setTextColor(80, 80, 80);
        doc.text(`Assigned To: ${safeString(repair.employee_id?.name || repair.employee_name)}`, 14, 120);
        doc.text(`Receiving Date: ${formatDate(repair.receiving_date)}`, 14, 126);
        if (repair.delivery_date) {
          doc.text(`Delivery Date: ${formatDate(repair.delivery_date)}`, 14, 132);
        }
        
        // Financial Summary - Manual table
        doc.setFontSize(11);
        doc.setTextColor(40, 40, 40);
        doc.text("FINANCIAL SUMMARY", 14, 142);
        ...
        doc.text("For any queries, contact: support@jewelryrepair.com", 105, pageHeight - 15, { align: 'center' });
                  ...
                  <div className="col-md-3 mb-3">
                    <div className="text-center p-3 border rounded bg-light">
                      ...
                      <div className="fw-bold mt-2">
                        {(() => {
                          const receivingDate = repair.receiving_date ? new Date(repair.receiving_date) : new Date();
                          const now = new Date();
                          const diffTime = Math.abs(now - receivingDate);
