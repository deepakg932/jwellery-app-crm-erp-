  ...
  try {
    ...
    if (existingCategory) {
      return res
        ...
        .json({
          success: false,
          message: "Inventory Category name already exists",
        });
    }
...
export const updateInventoryCategory = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(categoryId)) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid inventory category id" });
    }
    ...
    if (!updatedCategory) {
      return res
        .status(404)
        .json({ success: false, message: "Inventory Category not found" });
    }
